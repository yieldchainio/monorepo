...
export function removeUnnecessarySwaps(uprootTree: YCStep) {
  uprootTree.map((step) => {
...
function forceOrphansAdoption(deadParent: YCStep) {
  const grandParent = deadParent.parent;
  if (!grandParent) return; // They will end up on the streets
  for (const child of deadParent.children) grandParent.children.push(child);
}
