...
export const MainBinarySearch = async (_provider, _executor, _transactionFunction, _contractAddress, _func, _args, _key, // Key being iterated over / simulated right now
isCallback, _upperBound, _lowerBound, _tokensList, _divideBy) => {
    /**************************************************************** */
    let transactionFunction = _transactionFunction;
    let key = _key;
    let upperBound = _upperBound;
    let lowerBound = _lowerBound;
    let attempts = []; // Array of all the attempts made
    let divideBy = _divideBy;
    let hasAmountArg = _args.some((arg) => arg.parameter_identifier == key);
    /*****************************************************************/
    ...
    for (let i = 0; i < 1000; i++) {
        ...
        try {
            ...
            result = await searchTxidShortcut(_provider, _contractAddress, _func, "callback_post", args, abi, _executor, transactionFunction);
            let poo = 5;
        }
        catch (e) {
            // Catching potential errors. This may happen if transactions go through and there are call exceptions
            console.error("Simulation Failed In Main Binary Search Search, On Func:", _func.name, e.message);
        }
        // Checking if the simulation succeeded
        let didSucceed = result && result.logs && result.logs.length > 0 ? true : false;
        ...
        attempts.push({
            ...
            reproductionArgs: [
                ...
                [
                    "callback_post",
                    [
                        ...(await Promise.all(args.map(async (arg) => await encoding(_provider, _contractAddress, arg.value, arg.solidity_type, abi)))),
                    ],
                ],
                _executor,
            ],
        ...
        let pastSuccesses = attempts.filter((attempt) => attempt.success);
        ...
        if (Number(upperBound - lowerBound) < 1000000000000n &&
            pastSuccesses.length > 2) {
            console.log(`Difference is minor in func ${_func.name}, breaking Main Binary Search loop}`);
            break;
        }
        // Breaking the loop if the function is callback (Should not need to do binary search in that case)
        if (isCallback) {
            console.log(`Func ${_func.name} is callback, breaking Main Binary Search loop`);
            break;
        }
        // If it did not succeed, we lower the upperbound to the midpoint (in the process of narrowing down the range)
        if (!didSucceed) {
            console.log(`Failure in Main Binary Search On Func ${_func.name}, Amount: ${midPoint}, Iteration: ${i}, Arguments:`, `Logs In Current Receipt: ${result?.logs?.length}`);
            ...
            console.log(`Success in Main Binary Search On Func ${_func.name}, Amount: ${midPoint}, Iteration: ${i}, Arguments:`, `Logs In Current Receipt: ${result?.logs?.length}`);
            lowerBound = midPoint;
    ...
    let amountToSet = 0n;
    // Only successes
    let successes = attempts.filter((attempt) => attempt.success === true);
    // The largest success from all of the attempts
    let largestSuccess = null;
    let smallerSuccessForTest = null;
    try {
        largestSuccess = successes.reduce((prev, current) => prev.amount > current.amount ? prev : current);
        // A successfull attempt that is smaller by it's amount input than the largest one
        smallerSuccessForTest =
            successes.find((attempt) => attempt.amount < (largestSuccess?.amount || -1)) || null;
    }
    catch (e) {
        console.error("No Successes Found In Main Binary Search For Function:", _func.name);
    }
    /**
     ...
     */
    if (!largestSuccess) {
        console.log("No Successes Found In Main Binary Search For Function:", _func.name);
    ...
    if (amountToSet !== toBigInt(-1) && largestSuccess) {
        amountToSet = toBigInt(parseInt((Number(largestSuccess.amount) / (100 / divideBy)).toString()));
    }
    // The mapping of token addresses to whether or not they belong to the entire vault, or are already calculated
    // By the user's share
    let sharesCalculatedMapping = new Map();
    ...
    if (largestSuccess) {
        for (const [tokenAddress, largestTokenBalance] of largestSuccess.tokens) {
            ...
            if (largestTokenBalance === undefined ||
            ...
            let difference = toBigInt(largestTokenBalance) - toBigInt(smallerSuccessTokenBalance);
            ...
            let isCalculatedByShare = Number(difference) > 10000000n ? true : false;
    ...
    let keepFunction = amountToSet === toBigInt(-1) ? false : true;
