...
export default class GenericOrchestrator {
    ...
    shouldTxnBeIgnored(_log) {
        // Check if message is a log - ignore it if it is not.
        let log = this.getOnchainLog(_log);
        if (!log)
            return false;
        // If the TXID is in the ignore list, then ignore it
        if (this.txidsToIgnore.includes(log.transactionHash))
            return true;
        else {
        ...
        try {
            ...
            for (const message of Messages) {
                ...
                if (!this.shouldTxnBeIgnored(message.Body)) {
                    ...
                    console.log("About To call handler in .listen", Math.floor(Date.now() / 1000));
    ...
    collectGarbage = async () => {
        await new Promise((resolve) => {
            this.txidsToIgnore = this.txidsToIgnore.filter((txn) => !this.processedTxns.includes(txn));
            this.processedTxns = [];
...
//# sourceMappingURL=sqs-class.js.map