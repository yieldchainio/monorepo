...
const getABI = async () => {
    ...
    const functions = await (await axios.get("https://api.yieldchain.io/functions")).data.functions;
    // Adding the missing functions to the ABI
    const newAbi = typeof abi === "string" ? [...JSON.parse(abi)] : [...abi];
    // Adding the missing functions to the ABI in a loop
    for (const func of functions) {
        // Checking if the function is already included in the ABI
        const isAlreadyIncluded = abi.find((f) => f.name == `func_${func.function_identifier}`);
...
//# sourceMappingURL=getStrategyABI.js.map