...
import { getFunctionDetails, getParameterDetails, } from "./utils.js";
import { createArrayByNumberInput } from "./utils.js";
// Generates The Steps For The Strategy
export const GenerateSteps = async (_file_name, _strategy_obj) => {
    ...
    const GenerateStepDetails = async () => {
        ...
        for await (const step of steps_array) {
            ...
            let args = await Promise.all(fullFunc.arguments.map(async (arg) => await getParameterDetails(arg)));
            ...
            if (step_additional_args_arr.length > 0) {
                ...
                const appendArgToArray = async (_arg, index, array, plain) => {
                    ...
                    try {
                        index !== 0
                            ? await appendSameLine(`, abi.encode(${Web3.utils.toChecksumAddress(_arg)})`)
                            : await appendSameLine(`abi.encode(${Web3.utils.toChecksumAddress(_arg)})`);
                    }
                    catch {
                        index !== 0
    ...
    const GenerateStrategyRun = async () => {
        ...
        for await (const step of steps_body_array) {
            ...
            if (!(await getFunctionDetails(step.function_identifiers[0])).is_callback) {
                ...
                await appendNewLineInterFunction(`func_${function_id}("runStrategy_${current_strategy_run + 1}", [`);
                // Append The "Do Not Use Params" byte Into the function call, as many times as the function has parameters
                for await (const param of await createArrayByNumberInput(paramsAmount)) {
                    let fakeParamsArr = await createArrayByNumberInput(paramsAmount);
                    let paramindex = fakeParamsArr.indexOf(param);
                    paramindex !== 0
                        ? await appendSameLine(`, abi.encode("donotuseparamsever")`)
    ...
    const GenerateReverseFunctions = async () => {
        ...
        const doesHaveCounterFunction = async (_functionId) => {
            let _funcDetails = (await axios.get(`https://api.yieldchain.io/functions`)).data.functions.find((func) => func.function_identifier == _functionId);
