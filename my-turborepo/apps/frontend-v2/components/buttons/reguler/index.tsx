...
export const RegulerButton = forwardRef<HTMLDivElement, RegulerButtonProps>(
  ({ onClick, children, className }: RegulerButtonProps, ref) => {
    ...
    return (
      <div
        className={
          "w-max overflow-hidden h-max bg-custom-dropdown bg-opacity-80 flex items-center justify-between border-[#47474B] border-[1px] rounded-xl py-3 px-4 gap-4 select-none cursor-pointer hover:bg-opacity-70 hover:border-[#4F4F55] transition duration-200 ease-in-out font-athletics text-custom-textColor" +
          (" " + (className || ""))
        }
