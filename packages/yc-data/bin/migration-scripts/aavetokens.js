...
const provider = new ethers.JsonRpcProvider("https://arb-mainnet.g.alchemy.com/v2/Bva4kx5jvnUcfxDPwzW_iTO94JAw3ACP");
const contract = new ethers.Contract(aaveDataProvider, abi, provider);
const tokens = await contract.getAllReservesTokens();
console.log(tokens);
...
for (const token of tokens) {
    const existingToken = existingTokens.find((currToken) => currToken.address.toLowerCase() == token.tokenAddress.toLowerCase());
    if (existingToken) {
        console.log("Existing Token", existingToken);
        tokensToUpdate.push(existingToken.id);
    }
    else {
        const tokenContract = new ethers.Contract(token.tokenAddress, ["function decimals() view returns (uint256 decimals)"], provider);
