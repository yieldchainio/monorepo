...
const simulateFunction = async (
  ...
  let isCallback: boolean = _func.is_callback;

  // A mapping of tokens to their balances before anything happpens
  let preTokensMapping: TokenBalancesMapping = new Map();

  // Pushing balances to the mapping
  for await (const token of _tokensList) {
    preTokensMapping.set(
      token.address,
      await new ethers.Contract(token.address, erc20abi, _executor).balanceOf(
        _contractAddress
      )
    );
  }
  ...
  let shouldSimulateCallback: boolean = false;
  ...
  if (isCallback) {
    ...
    if (!shouldSimulateCallback) {
      let balanceBooleanMapping: SharesCalculatedMapping = new Map();
      for (const tokenAddress of _tokensList) {
        balanceBooleanMapping.set(tokenAddress.address, false);
      }
      return {
        argsForCall: await Promise.all(
          _args.map(
            async (arg: ExtendedArgument) =>
              await encoding(
                _provider,
                _contractAddress,
                arg.value,
  ...
  let amountArgMapping = new Map<number, any>();
  ...
  let sharesCalculatedTokensMapping: SharesCalculatedMapping = new Map();
  ...
  for await (const arg of _args) {
    try {
      ...
      if (typeof arg.value === "string" && arg.value.includes("_BALANCE")) {
        ...
        if (arg.value.find((_arg: any) => _arg.includes("_BALANCE")))
          amountArgMapping.set(arg.parameter_identifier, arg.value);
      }
    } catch (e: any) {
  ...
  let transactionFunction: SendRawTransaction | CallbackTransaction =
    shouldSimulateCallback ? callCallbackFunction : sendRawTransaction;
  ...
  let iterationTimes;
  if (mappingIterator.length > 0) iterationTimes = mappingIterator.length;
  else iterationTimes = 1;

  // Starting the iteration on each amount argument
  for (let i = 0; i < iterationTimes; i++) {
    // The key of the argument in the mapping
    let key: number = mappingIterator[i] ? mappingIterator[i][0] : -1;

    // Starting Upperbound & Lowerbounds
    let upperBound: bigint = 1000000000000000000000000n;
    let lowerBound: bigint = 10000000000000000n;
    ...
    if (!isCallback) {
      ({ upperBound, lowerBound } = await SearchCoarseUpperbound(
        _provider,
        _executor,
        transactionFunction,
        _contractAddress,
    ...
    // We do not want to simulate upperbounds for callback
    if (!isCallback) {
      ({ upperBound, lowerBound } = await SearchCoarseLowerbound(
        _provider,
        _executor,
        transactionFunction,
        _contractAddress,
     ...
     */
    let { amountToSet, sharesCalculatedMapping, keepFunction } =
      await MainBinarySearch(
        _provider,
        _executor,
        transactionFunction,
        _contractAddress,
      ...
      _args.map(async (arg) => {
        if (amountArgMapping.has(arg.parameter_identifier)) {
          ...
          const value = !Array.isArray(arg.value)
            ? setAmount
            : arg.value.map((arg: any) =>
                typeof arg == "string" && arg.includes("_BALANCE")
                  ? setAmount
                  : arg
              );
