 ...
 */
const getTokenAmountToTransfer = async (_trueFalseMapping, _preChangesMapping, _tokensToTransferAmount, _func, _provider, _withdrawShare) => {
    ...
    for await (const [tokenAddress, alreadyCalculated] of trueFalseMapping) {
        ...
        let change = preChangesMapping.get(ethers.getAddress(tokenAddress)) || "0";
        console.log("iterating on token address", tokenAddress, "Inide Func " + func.name, "change:", change);
        // The previous amount to transfer to the user (if any, so that we can add to it rather than override it)
        let prevTransferAmt = tokensToTransferAmount.get(ethers.getAddress(tokenAddress));
        let symbol = await new ethers.Contract(tokenAddress, erc20abi, originalProvider).symbol();
        console.log("Inside true false mapping, symbol: ", symbol, `prev transfer amount: ${prevTransferAmt}`);
        ...
        let amtToSet = (toBigInt(prevTransferAmt || 0) +
            toBigInt(change) /
                toBigInt(parseInt((alreadyCalculated ? 1 : 100 / withdrawShare).toString()))).toString();
        // Setting the amount to transfer to the user
        tokensToTransferAmount.set(ethers.getAddress(tokenAddress), amtToSet);
        console.log(`Amt to set for token ${symbol}: ${amtToSet} after function ${func.name}`);
    }
    return tokensToTransferAmount;
};
export default getTokenAmountToTransfer;
//# sourceMappingURL=get-token-amt-to-transfer.js.map