...
export const SearchCoarseUpperbound = async (_provider, _executor, _transactionFunction, _contractAddress, _func, _args, _key, // Key being iterated over / simulated right now
_upperBound, _lowerBound) => {
    ...
    for (let i = 0; i < 100; i++) {
        ...
        try {
            ...
            console.error("Simulation Failed In Upperbound Corase Search, On Func:", _func.name, e);
        }
        ...
        if (result && result.logs && result.logs.length > 0) {
            console.log(`Success in Upperbound On Func ${_func.name}, Amount: ${amtAttempt}, Iteration: ${i}, Arguments:`, `Logs In Current Receipt: ${result.logs.length}`);
            ...
            console.log(`Failure in Upperbound On Func ${_func.name}, Amount: ${amtAttempt}, Iteration: ${i}, Arguments:`, `Logs In Current Receipt: ${result?.logs?.length}`);
            break;
        }
...
//# sourceMappingURL=coarse-upperbound.js.map