...
export const SearchCoarseLowerbound = async (
  ...
  for (let i = 0; i < 100; i++) {
    ...
    let amtAttempt: bigint = lowerBound;
    ...
    let result: EthersReceipt | ExtendedReceipt | null = null;
    ...
    try {
      ...
      console.error(
        "Simulation Failed In Lowerbound Corase Search, On Func:",
        _func.name,
        e
      );
    }
    ...
    if (result && result.logs && result.logs.length > 0) {
      console.log(
        `Success in Lowerbound On Func ${_func.name}, Amount: ${amtAttempt}, Iteration: ${i}`,
      ...
      console.log(
        `Failure in Lowerbound On Func ${_func.name}, Amount: ${amtAttempt}, Iteration: ${i}, Arguments: `,
