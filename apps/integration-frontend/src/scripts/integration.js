...
exports.integrateIntoDB = void 0;
const axios_1 = __importDefault(require("axios"));
const ethers_1 = require("ethers");
const integrateIntoDB = async (_integrationObject) => {
    try {
        let oldFuncIdsToDbFuncIds = new Map();
        let funcIds = [];
        let oldFuncIdsToOldFuncObjs = new Map();
        for (let i = 0; i < _integrationObject.functions.length; i++) {
            ...
            oldFuncIdsToOldFuncObjs.set(currentFunction.tempId, _integrationObject.functions[i]);
            ...
            for (let j = 0; j < currentFunction.args.length; j++) {
                ...
                let newParamId = await (await axios_1.default.post("https://integrationapi.yieldchain.io/add-parameter", argAddingObject)).data.parameter_identifier;
            ...
            for (let j = 0; j < currentFunction.flows.length; j++) {
                // Current flow
                let currentFlow = currentFunction.flows[j];
                let tokenId = currentFlow.token_details.token_identifier;
                if (tokenId === null) {
                    alert("Error: Token ID is null, Current Flow: " + currentFlow);
                    return;
                }
                // The object we're going to post when adding the flow
                let flowAddingObject = {
                ...
                let newFlowId = await (await axios_1.default.post("https://integrationapi.yieldchain.io/add-flow", flowAddingObject)).data.flow_identifier;
            ...
            // Posting the function to be added to the database. Being returned the function ID (default auto-incrmeneting ID)
            let newFuncId = await (await axios_1.default.post("https://integrationapi.yieldchain.io/add-function", funcObjectToPost)).data.function_identifier;
        ...
        for (let i = 0; i < _integrationObject.functions.length; i++) {
            ...
            let newCounterFuncId = oldFuncIdsToDbFuncIds.get(currentFunction.counterFuncId || -1);
            // Getting the new unlocked by func ID
            let newUnlockedByFuncId = oldFuncIdsToDbFuncIds.get(currentFunction.unlockedByFuncId || -1);
            ...
            if (Object.keys(postObject).length > 0) {
                let res = await axios_1.default.post(`https://integrationapi.yieldchain.io/update-function/${oldFuncIdsToDbFuncIds.get(currentFunction.tempId)}`, postObject);
        ...
        let protocolsChains = await (await axios_1.default.get("https://api.yieldchain.io/protocols-networks")).data.protocols_networks;
        // Rows of Protocol IDs => Address IDs
        let protocolsAddresses = await (await axios_1.default.get("https://api.yieldchain.io/protocols-addresses")).data.protocols_addresses;
        // Rows of DBAddresses
        ...
        let currentAddress = allAddresses.find((address) => ethers_1.ethers.getAddress(address.contract_address) ===
            ethers_1.ethers.getAddress(_integrationObject.contract_address));
        // The current address ID (if it exists)
        let currentAddressId = currentAddress === null || currentAddress === void 0 ? void 0 : currentAddress.address_identifier;
        // Add the address to the DB if it doesn't exist
        if (currentAddress == undefined) {
            // Post req to add it
            let newAddress = await axios_1.default.post("https://integrationapi.yieldchain.io/add-address", {
        ...
        let currentAddressInProtocolPairs = protocolsAddresses.find((protocol_address_pair) => {
            return (protocol_address_pair.protocol_identifier ===
        ...
        if (!currentAddressInProtocolPairs) {
            await axios_1.default.post("https://integrationapi.yieldchain.io/add-protocol-address", {
        ...
        let currentProtocolChainPair = protocolsChains.find((protocol_chain_pair) => {
            return (protocol_chain_pair.protocol_identifier ===
                _integrationObject.protocol_identifier &&
        ...
        if (!currentProtocolChainPair) {
            await axios_1.default.post("https://integrationapi.yieldchain.io/add-protocol-network", {
                protocol_identifier: _integrationObject.protocol_identifier,
                chain_id: _integrationObject.chain_id,
            });
        }
        ...
        for (const fullFunc of _integrationObject.functions) {
            ...
            await axios_1.default.post(`https://integrationapi.yieldchain.io/add-function-to-action/`, postObject);
        }
        return true;
    }
    catch (e) {
...
exports.integrateIntoDB = integrateIntoDB;
