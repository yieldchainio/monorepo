...
import { getFunctionDetails, getParameterDetails, getAddressDetails, getFunctionAddress, } from "./utils.js";
export const generateFunctions = async (_fileName, _strategy_obj) => {
    ...
    ///////////////////////////////////////////////////////////////////////
    ///////////////////////Database Context/////////////////////////////////
    let DatabaseContext = await GetDatabaseContext();
    ...
    let counterFunctionsArr = await Promise.all(steps_array.map(async (step) => {
        let counterFunctionIdentifier = await full_functions_list.find((func) => func.function_identifier ==
            full_functions_list.find((tFunc) => tFunc.function_identifier == step.function_identifiers[0]).counter_function_identifier);
        let addressIdentifier;
        try {
            addressIdentifier = (await getFunctionAddress(counterFunctionIdentifier.function_identifier)).address_identifier;
        }
        catch (e) {
    ...
    const fetchDetails = async (_arr) => {
        for await (const step of _arr) {
            if (step.function_identifiers[0] == null)
                break;
            for await (const funcid of step.function_identifiers) {
                ...
                console.log(step.address_identifiers[funcindex], "Step Address Ientifier", "Func ID of it", step.function_identifiers[0]);
                let funcaddress = await (await getAddressDetails(step.address_identifiers[funcindex])).contract_address;
                let paramsArr = [];
                functions_arr.push(funct);
                for await (const paramid of funct.arguments) {
                    let param = await getParameterDetails(paramid);
    ...
    await fetchDetails(counterFunctionsArr.filter((counterFunc) => counterFunc.function_identifiers[0] != null));
    functions_arr = await functions_arr.filter((functionObj, index) => functions_arr.findIndex((t) => t.function_identifier == functionObj.function_identifier) == index);
    /**
     * @MainFunctionGen
    ...
    for await (const func of functions_arr) {
        ...
        let func_address = Web3.utils.toChecksumAddress(functions_to_addresses_mapping.get(func.function_identifier));
        ...
        const generateRegularFunctioNWrapper = async () => {
            spacing();
            await appendNewLineInterContract(`function func_${func.function_identifier}`);
            await appendSameLine(`(string memory _funcToCall`);
            await appendSameLine(`, bytes[${func.number_of_parameters || 1}] memory _arguments`);
            await appendSameLine(`) public onlyExecutor {`);
            // Address For Function To Call
            await appendNewLineInterFunction(`address currentFunctionAddress = ${func_address};`);
            // Checks Whether Params Provided Were Default Or Not, Assigns Boolean Based Off Of that
            await appendNewLineInterFunction(`bool useCustomParams = keccak256(_arguments[0]) == keccak256(abi.encode("donotuseparamsever")) ? false : true;`);
            ...
            const addCustomArgsCall = async () => {
                ...
                await appendNewLineInterFunction2x(`currentFunctionAddress.call{value: 0}(abi.encodeWithSignature("${func_name}(`);
                for await (const arg of func_args) {
                    let argindex = func_args.findIndex((param) => arg.parameter_identifier == param.parameter_identifier);
                ...
                for await (const arg of func_args) {
                    let argindex = func_args.findIndex((param) => arg.parameter_identifier == param.parameter_identifier);
                    await appendSameLine(`, abi.decode(_arguments[${argindex}], (${arg.solidity_type}))`);
                }
                await appendSameLine(`));`);
                return;
            };
            ...
            const addDefaultArgsCall = async () => {
                ...
                await appendNewLineInterFunction2x(`(success, result) = `);
                await appendSameLine(`currentFunctionAddress.call{value: 0}(abi.encodeWithSignature("${func_name}(`);
                for await (const arg of func_args) {
                    let argindex = func_args.findIndex((param) => arg.parameter_identifier == param.parameter_identifier);
                    if (argindex == func_args.length - 1) {
                        await appendSameLine(`${arg.solidity_type}`);
            ...
            await appendNewLineInterFunction(`require(success, "Function Call Failed On func_${func_id}, Strategy Execution Aborted");`);
        ...
        const generateCallBackFunction = async () => {
            ...
            const createPreCallbackFunction = async () => {
                ...
                await appendNewLineInterFunction(`if (useCustomParams) {`);
                const addCustomArgsCall = async () => {
                    await appendNewLineInterFunction2x(`bytes[] memory eventArr = new bytes[](${func_args.length});`);
                    await appendNewLineInterFunction2x(`bytes[${func_args.length}] memory eventArrFixed = [`);
                    for await (const arg of func_args) {
                        let argindex = func_args.findIndex((param) => arg.parameter_identifier == param.parameter_identifier);
                        if (argindex == 0) {
            ...
            eventArr[i] = eventArrFixed[i];
                    ...
                    await appendNewLineInterFunction2x(`emit CallbackEvent("${func_name}", _funcToCall, eventArr);`);
                    return;
                };
                /**
                 * Generates Delegate Call For The Case Where There Were @NO Custom Arguments Provided Into The Wrapping Function,
