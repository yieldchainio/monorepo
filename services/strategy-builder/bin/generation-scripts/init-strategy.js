...
import { getFunctionDetails, getFlowDetails, getTokenDetails, getParameterDetails, createArrayByNumberInput, } from "./utils.js";
export const initStrategy = async (_strategy_obj) => {
    /**
    ...
    let vault_deposit_token = await getTokenDetails(vault_deposit_token_identifier);
    let base_steps = init_strategy_obj.base_steps;
    let base_steps_functions_details = await base_steps.map(async (base_step_item) => (base_step_item = await getFunctionDetails(base_step_item.function_identifier)));
    ...
    const generateAdditionalBoilerplate = async () => {
        // Variables
        await appendNewLineInterContract(`IERC20 public vault_deposit_token = IERC20(${Web3.utils.toChecksumAddress(vault_deposit_token.address)});`);
      ...
      // The Custom Arguments Are Used To Store Any Custom Arguments That The Creator May Want To Pass To The Step`);
        ...
        const generateInitialDepositFunction = async (_base_step, _wasInitialGenerated) => {
            ...
            let flow_ids_arr = (await getFunctionDetails(base_step.function_identifiers[0])).flows;
            let outflows_arr = [];
            let step_func_details = await getFunctionDetails(base_step.function_identifiers[0]);
            ...
            for await (const outflowObj of outflows_arr) {
                (await getTokenDetails(outflowObj.token_identifier)).address.toLowerCase() !== vault_deposit_token.address.toLowerCase()
            ...
            if (!_wasInitialGenerated) {
                ...
                if (is_current_last) {
                    ...
                    for await (const outflow_obj of outflows_arr_no_initial) {
                        let token_obj = (await getTokenDetails(outflow_obj.token_identifier));
                        ...
                        await appendNewLineInterFunction(`to_tokens_arr[${current_index}] = ${Web3.utils.toChecksumAddress(token_obj.address)};`);
                    }
                    await appendNewLineInterFunction(`bytes[] memory depositEventArr = new bytes[](6);`);
                    await appendNewLineInterFunction(`bytes[6] memory depositEventArrFixed = [abi.encode(currentChainId), abi.encode(${Web3.utils.toChecksumAddress(vault_deposit_token.address)}), abi.encode(to_tokens_arr), abi.encode(_amount), abi.encode(to_tokens_divs_arr), abi.encode(address(this))];`);
            ...
            // Triggers "Base Strategy" (Swaps + Base Steps)`);
                ...
                let fakeparamsarr = await createArrayByNumberInput(step_func_details.number_of_parameters);
    ...
    for await (const pool of init_strategy_obj.base_steps) {
        let res = await axios.get(`https://api.yieldchain.io/address-flows/${pool.address_identifiers[0]}`);
