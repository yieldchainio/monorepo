...
----------------------------------------------------------------*/
export const getBalancesChanges = async (_provider, _contractAddress, _tokensList, _transactionToExecute, _executor, _ignoreTxns) => {
    // Mapping of all the token balances before the transaction
    let preTokensMapping = await getTokenBalances(_provider, _contractAddress, _tokensList);
    /**
    ...
    if (_transactionToExecute.function.is_callback) {
        ...
        await callCallbackFunction(_provider, _transactionToExecute.to, _transactionToExecute.function, _transactionToExecute.args, _executor, _ignoreTxns);
    }
    else {
        // Otherwise if a regular function, execute it
        await sendRawTransaction(_provider, _transactionToExecute.to, _transactionToExecute.function, _transactionToExecute.args, _executor);
    }
    // Mapping of token balances post the function call
    let postTokensMapping = await getTokenBalances(_provider, _contractAddress, _tokensList);
    // A mapping of the differences between the balances before and after the function call
    let tokenBalancesChanges = new Map();
    ...
    for await (const token of _tokensList) {
        ...
        let change = toBigInt(postBalance.toString()) - toBigInt(preBalance.toString());
        /**
         * @notice
         ...
         */
        let tokenContract = new ethers.Contract(ethers.getAddress(token.address), erc20abi, _provider);
        if (_transactionToExecute.function.name === "func_17") {
            console.log(`Inside iteration in Lifi Func_17, getBalancesChanges, over token ${await tokenContract.symbol()}, change is ${change.toString()}`);
        }
        tokenBalancesChanges.set(ethers.getAddress(token.address), change.toString());
    }
    return tokenBalancesChanges;
};
//# sourceMappingURL=GetTokenBalanceChanges.js.map