export function formatInterval(executionInterval) {
    let minute = 60;
    let hour = 3600;
    let day = 86400;
    let week = 604800;
    ...
    if (executionInterval >= month) {
        const interval = parseFloat((executionInterval / month).toFixed(1).toString().split(".")[1]) > 0
    ...
    if (executionInterval >= week) {
        const interval = parseFloat((executionInterval / week).toFixed(1).toString().split(".")[1]) > 0
    ...
    if (executionInterval >= day) {
        const interval = parseFloat((executionInterval / day).toFixed(1).toString().split(".")[1]) > 0
    ...
    if (executionInterval >= hour) {
        const interval = parseFloat((executionInterval / hour).toFixed(1).toString().split(".")[1]) > 0
    ...
    if (executionInterval >= minute) {
        const interval = parseFloat((executionInterval / minute).toFixed(1).toString().split(".")[1]) > 0
