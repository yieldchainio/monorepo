...
export function SlideShow(sliderProps: Settings) {
                  ...
                  if (React.isValidElement(dot)) {
                    return (
                      <dot.type
                        {...(dot.props || {})}
                        key={dot.key}
                        className={(dot.props as any)?.className}
                      >
                        {React.Children.map(
                          (dot.props as any).children,
                          (child) => {
                            return (
                              <child.type
                                {...child.props}
                                className={
                                  ...
                                  " " +
                                  (dot.props &&
                                  (dot.props as any).className.includes(
                                    "slick-active"
                                  )
                                    ? ""
                                    : "bg-opacity-50")
