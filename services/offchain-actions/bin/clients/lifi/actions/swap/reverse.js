...
export const lifiSwapReverse = async (functionRequest, provider) => {
    const toToken = AbiCoder.defaultAbiCoder().decode(["address"], functionRequest.args[0])[0];
    const fromToken = AbiCoder.defaultAbiCoder().decode(["address"], functionRequest.args[1])[0];
