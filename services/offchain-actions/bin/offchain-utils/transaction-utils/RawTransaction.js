...
export const sendRawTransaction = async (_provider, // Ethers Provider Class
_contractAddress, // A checksum address (e.g, 0x73112...94FaJK12)
_func, // A Simplified function object, or a string
_args, // Arguments of any type to spread into the function call
_executor, // Ethers Wallet Class
_try = 0, _optionalNonce, // An optional nonce incase there are issues with nonce managemnt/some errors are caught
_ignoreTxns // An optional GenericOrchestrator to ignore transactions
) => {
    ...
    let contract = new ethers.Contract(_contractAddress, abi, _executor);
    // The function name - we may get a raw function name or a "SimplfiedFunction" type object,
    // With a name proprety on it.
    let functionName = typeof _func === "object" ? _func.name : _func;
    // An unsigned transaction data populated using the arguments
    let unsignedtx = await contract[functionName].populateTransaction(..._args); // An Unsigned Transaction Populated
    let nonceUsed = _optionalNonce ||
    ...
    try {
        ...
        const tx = await _executor.sendTransaction(txnArguments);
        ...
        // Getting the receipt
        let receipt = await _provider.getTransactionReceipt(tx.hash);
        ...
        if ((e.message.includes("nonce") || e.message.includes("Nonce")) &&
            ...
            return await sendRawTransaction(_provider, _contractAddress, _func, _args, _executor, _try + 1, _try < 10 ? nonceUsed : nonceUsed - 1);
        }
        ...
        if (e?.error?.code === -32000) {
            ...
            return await sendRawTransaction(_provider, _contractAddress, _func, _args, _executor, _try + 1, _try < 10
                ? nonceUsed
                : (await _provider.getTransactionCount(_executor.address, "pending")) + 1);
        }
        // If the error is a nonce too low error, we can try to send the transaction again with a higher nonce
        if (e.code.includes("NONCE_EXPIRED")) {
            console.log("NONCE EXPIRED");
            return await sendRawTransaction(_provider, _contractAddress, _func, _args, _executor);
        }
