...
----------------------------------------------------------------*/
export const reverseStrategy = async (_inputtedProvider, _contractAddress, _funcToCall, _args_bytes_arr) => {
    /**
     * @Network JSON RPC
     */
    let networkJsonRpc = _inputtedProvider;
    /**
     * @Ethers Provider - Origin Chain
     */
    let originalProvider = new ethers.JsonRpcProvider(networkJsonRpc);
    // The chain ID
    let _chainId = Number((await originalProvider.getNetwork()).chainId);
    // The transaction hash of the transaction that WILL be called after the reverse-strategy is done.
    let withdrawPostTxid = null; // TODO: Change the type
    ...
    let forkProvider = new ethers.JsonRpcProvider(forkJsonRpc, undefined, {
        polling: true,
    });
    // Executor signer (Fork)
    let forkExecutor = new EthersExecutor("0x" + process.env.PRIVATE_KEY, forkProvider);
    // Executor signer (Origin)
    let executor = new EthersExecutor("0x" + process.env.PRIVATE_KEY, originalProvider);
    // Strategy's contract instance
    let contract = new ethers.Contract(_contractAddress, abi, executor);
    console.log("Before calling deposit token");
    // Vault deposit token
    let depositToken = await contract.vault_deposit_token();
    // The withdrawing user's address
    let userAddress = ethers.AbiCoder.defaultAbiCoder().decode(["address"], _args_bytes_arr[0])[0];
    // The amount that the user wants to withdraw (Already validated they have enough on-chain)
    let withdrawAmount = ethers.AbiCoder.defaultAbiCoder().decode(["uint256"], _args_bytes_arr[1])[0];
    // Total shares of the vault
    let totalShares = (await contract.totalVaultShares()) + withdrawAmount;
    ...
    let depositTokenContract = new ethers.Contract(depositToken, erc20abi, originalProvider);
    ...
    let formattedWithdrawAmount = Number(ethers.formatUnits(withdrawAmount, await depositTokenContract.decimals()));
    // the total shares formatted by the decimals, for non-bigint calculation
    let formattedTotalShares = Number(ethers.formatUnits(totalShares, await depositTokenContract.decimals()));
    // The share of the user in % of the vault, including the share that he wants to withdraw out of his share (e.g 50% of his share)
    let withdrawShare = ((formattedWithdrawAmount * 100) / (formattedTotalShares * 100)) * 100;
    // The array of the function identifiers of the reverse strategy
    let reverseFunctionsArray = await Promise.all(ethers.AbiCoder.defaultAbiCoder()
        .decode(["uint256[]"], _args_bytes_arr[2])[0]
        .map(async (funcId) => {
        return await getFullFunc(Number(funcId));
    }));
    // The array of step IDs of the strategy, reversed
    let reverseStepsArray = await stepIDsToSteps(ethers.AbiCoder.defaultAbiCoder().decode(["uint256[]"], _args_bytes_arr[3])[0], originalProvider, _contractAddress);
    ...
    let userPreShares = ethers.AbiCoder.defaultAbiCoder().decode(["uint256"], _args_bytes_arr[4])[0];
    ...
    try {
        ...
        reverseFunctionsArray = await dbFuncsToSimplifiedFuncs(reverseFunctionsArray, // @Typeguard - must be DBFunction here (Converted to simplified function in the next line)
        reverseStepsArray, _contractAddress);
        ...
        let tokensToTransferAmount = new Map();
        // Array of all token addresses relating to the vault's strategy.
        let allTokens = await getStrategyTokens(_contractAddress, originalProvider);
        ...
        for (let i = 0; i < reverseFunctionsArray.length; i++) {
            ...
            let { argsForCall, sharesCalculatedMapping, shouldKeepFunction } = await simulateFunction(forkProvider, _contractAddress, func, func.arguments, allTokens, withdrawShare, forkExecutor);
            /**
            ...
            let transactionFunction = func.is_callback ? callCallbackFunction : sendRawTransaction;
            /**
             ...
             */
            await transactionFunction(forkProvider, _contractAddress, func, argsForCall, forkExecutor);
            await forkProvider.send("evm_mine", []); // Mine a block
            console.log(`Simulation Iteration On Function ${func.name} Completed, True-False Mapping Size: ${sharesCalculatedMapping.size}`);
        ...
        for await (const token of allTokens) {
            ...
            let tokenContract = new ethers.Contract(token.address, erc20abi, originalProvider);
            // The balance of the vault (of that certain token)
            let tokenBalance = await tokenContract.balanceOf(_contractAddress);
            ...
            console.log(`Balance of ${await tokenContract.symbol()} is ${tokenBalance} in initial Local balance iteration`);
            // The share of the user
            let userShareOfBalance = toBigInt(tokenBalance) / toBigInt(100 / withdrawShare);
            // To not deduct using negative numbers (User can not be in debt before executing any transactions)
            if (userShareOfBalance < 0n)
                userShareOfBalance = 0n;
            // Set the amount to transfer to the user
            tokensToTransferAmount.set(ethers.getAddress(token.address), userShareOfBalance.toString());
        }
        /**
         ...
         */
        let orchestrator = GenericOrchestrator.getSingleton();
        /**
         * @notice
        ...
        for (let i = 0; i < reverseFunctionsArray.length; i++) {
            ...
            tokensToTransferAmount = await getTokenAmountToTransfer(trueFalseMapping, preChangesMapping, tokensToTransferAmount, func, originalProvider, withdrawShare);
            console.log("tokensToTransferAmount After Calling Get Token Amount To Transfer", tokensToTransferAmount);
        }
        ...
        let transferTokensBiggerThanZero = [];
        ...
        let preSwapDepositTokenBalance = (await depositTokenContract.balanceOf(_contractAddress)).toString();
        console.log("Deposit Token Balance BEFORE", preSwapDepositTokenBalance);
        // The array of addresses to transfer to
        let transferAddressesArr = transferTokensBiggerThanZero.map((t) => t[0]);
        // The array of amounts to transfer
        let transferAmountsArr = transferTokensBiggerThanZero.map((t) => t[1]);
        /**
         * @notice Sending the callback_post transaction to the contract for swapping all allocated token balances to deposit token
         */
        let postTxidReceipt = await manyToOneSwap(networkJsonRpc, await contract.getAddress(), transferAddressesArr, transferAmountsArr, depositToken, executor);
        console.log("Sent Many-To-One Swap Transaction On Mainnet", postTxidReceipt?.hash);
        /**
        ...
        let currentDepositTokenBalance = (await depositTokenContract.balanceOf(_contractAddress)).toString();
        ...
        let doesHaveDepositTokenFromBefore = tokensToTransferAmount.get(depositToken) || "0";
        // Difference in deposit token balance after swaps (what the user is entitled for)
        let depositTokenAmount = (toBigInt(currentDepositTokenBalance) -
        ...
        let haveTransferred = false;
        // If the deposit token amount is bigger than 0, we transfer it to the user
        if (toBigInt(depositTokenAmount) > 0n) {
            console.log("Deposit Token Amount To Send Is Bigger Than 0, Amount: ", depositTokenAmount);
            // Withdrawing these amount of tokens to the user
            let transferTxid = await manualWithdraw(depositToken, await contract.getAddress(), userAddress, depositTokenAmount, originalProvider._getConnection().url);
            // This is the receipt for the transfer
            let transferTxidReceipt = await originalProvider.getTransactionReceipt(transferTxid.hash);
        ...
        withdrawPostTxid = await contract.withdraw_post(haveTransferred, userPreShares.toString(), userAddress.toString());
        const receipt = await originalProvider.waitForTransaction(withdrawPostTxid.hash);
        console.log("Sent Withdraw Post Transaction On Mainnet", withdrawPostTxid.hash);
        return receipt;
    }
    catch (e) {
        if (e.message !== "debug") {
            ...
            if (!postTxidReceipt || postTxidReceipt.status) {
                await contract.withdraw_post(false, userPreShares.toString(), userAddress.toString());
                console.log("Caught error, sent withdraw post on mainnet to revert withdrawl changes");
            }
        }
    }
};
