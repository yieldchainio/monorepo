...
import {
  ...
  getTokenDetails,
  getParameterDetails,
  createArrayByNumberInput,
} from "./utils.js";
import * as dbtypes from "../generation-types";

export const initStrategy = async (_strategy_obj: any) => {
  /**
  ...
  // TODO: AS Needed
  let vault_deposit_token_identifier =
    _strategy_obj.vault_deposit_token_identifier;
  let vault_deposit_token = await getTokenDetails(
    vault_deposit_token_identifier
  );
  let base_steps = init_strategy_obj.base_steps;
  let base_steps_functions_details = await base_steps.map(
    async (base_step_item: any) =>
      (base_step_item = await getFunctionDetails(
        base_step_item.function_identifier
      ))
  );
  ...
  const writeToFile = async (_fileName: any, _content: any) => {
    await fs.writeFileSync(_fileName, _content);
  };
  const appendToFile = async (_fileName: any, _content: any) => {
    await fs.appendFileSync(_fileName, _content);
  };
  const appendNewLineShortcut = async (_content: any) => {
  ...
  const appendNewLineShortcutFactory = async (_content: any) => {
    await appendToFile(factory_file_name, `\n${_content}`);
  };
  const appendNewLineInterContractFactory = async (_content: any) => {
    await appendToFile(factory_file_name, `\n    ${_content}`);
  };
  const appendNewLineInterFunctionFactory = async (_content: any) => {
    await appendToFile(factory_file_name, `\n        ${_content}`);
  };
  const appendNewLineInterFunction2xFactory = async (_content: any) => {
    await appendToFile(factory_file_name, `\n            ${_content}`);
  };
  const appendNewLineInterFunction3xFactory = async (_content: any) => {
    await appendToFile(factory_file_name, `\n                ${_content}`);
  };
  const spacing = async () => {
    await appendToFile(file_name, `\n\n`);
  ...
  const generateAdditionalBoilerplate = async () => {
    ...
    await appendNewLineInterContract(
      `IERC20 public vault_deposit_token = IERC20(${Web3.utils.toChecksumAddress(
    ...
    await appendNewLineInterContract(
      `string public strategyTitle = "${name}";`
    );
    ...
    await appendNewLineInterContract(
      `uint256 public interval = ${exec_interval};`
    );
    ...
    for await (const step of steps_array) {
      ...
      await appendNewLineInterFunction(
        `steps[${step.step_identifier}] = step_${step.step_identifier};`
      );
    }
    ...
    let outflows_arr_no_initial: any = [];

    const generateInitialDepositFunction = async (
      _base_step: any,
      _wasInitialGenerated: any
    ) => {
      // Initiation For Function-Scoped Variables
      let base_step = _base_step;
      let flow_ids_arr = (
        await getFunctionDetails(base_step.function_identifiers[0])
      ).flows;
      let outflows_arr: any = [];
      let step_func_details = await getFunctionDetails(
        base_step.function_identifiers[0]
      );
      ...
      let is_current_last =
        base_steps.findIndex(
          (t: any) => t.step_identifier == base_step.step_identifier
        ) ==
        base_steps.length - 1
          ? true
          : false;
      ...
      for await (const arg of step_func_details.arguments) {
        let argdetails = await getParameterDetails(arg);
        await step_func_params_arr.push(argdetails);
        await params_values_arr.push(argdetails.value);

        if (
          argdetails.value.split(" / activeDivisor")[0].split("_")[1] ==
          "BALANCE"
        ) {
      ...
      for await (const flow_id of flow_ids_arr) {
        await getFlowDetails(flow_id).then((flow_details_obj) => {
      ...
      for await (const outflowObj of outflows_arr) {
        (
          await getTokenDetails(outflowObj.token_identifier)
        ).address.toLowerCase() !== vault_deposit_token.address.toLowerCase()
      ...
      if (!_wasInitialGenerated) {
        ...
        if (is_current_last) {
          ...
          await appendNewLineInterFunction(
            `address[] memory to_tokens_arr = new address[](${outflows_arr_no_initial.length});`
          );

          // Array Of Divisors To Tokens It Will Swap To
          await appendNewLineInterFunction(
            `uint[] memory to_tokens_divs_arr = new uint[](${outflows_arr_no_initial.length});`
          );
          ...
          for await (const outflow_obj of outflows_arr_no_initial) {
            let token_obj: dbtypes.DBToken = (await getTokenDetails(
              outflow_obj.token_identifier
            )) as dbtypes.DBToken;
            let current_index: number = outflows_arr_no_initial.findIndex(
              (outflowObj: any) =>
                outflowObj.flow_identifier == outflow_obj.flow_identifier
            );
            await appendNewLineInterFunction(
              `to_tokens_divs_arr[${current_index}] = ${
                base_step.divisor * outflows_arr_no_initial.length
              };`
            );
            await appendNewLineInterFunction(
              `to_tokens_arr[${current_index}] = ${Web3.utils.toChecksumAddress(
                token_obj.address
              )};`
            );
          }
          ...
          await appendNewLineInterFunction(
            `bytes[6] memory depositEventArrFixed = [abi.encode(currentChainId), abi.encode(${Web3.utils.toChecksumAddress(
              vault_deposit_token.address
            )}), abi.encode(to_tokens_arr), abi.encode(_amount), abi.encode(to_tokens_divs_arr), abi.encode(address(this))];`
          );
        ...
        if (!wasPostDepositinit) {
          ...
          await appendNewLineInterContract(
            `uint256 PRE_BALANCE = ${vault_deposit_token.symbol}_BALANCE;`
          );
          await appendNewLineInterContract(`updateBalances();`);
          await appendNewLineInterContract(
            `uint256 POST_BALANCE = ${vault_deposit_token.symbol}_BALANCE;`
          );
          await appendNewLineInterFunction(
        ...
        for await (let balance_val of balances_params_arr) {
          ...
          if (balance_val.value.split("_")[0] == vault_deposit_token.symbol) {
            ...
            await appendNewLineInterFunction(
              `updateActiveStep(step_${base_step.step_identifier});`
            );

            await appendNewLineInterFunction(
              `uint256 currentIterationBalance = ${tokenSymbol}.balanceOf(address(this));`
            );
            await appendNewLineInterFunction(
            ...
            await appendNewLineInterFunction2x(
              `${tokenBalanceVar} = (POST_BALANCE - PRE_BALANCE) * activeDivisor;`
            );
            await appendNewLineInterFunction(
              `} else if (currentIterationBalance < POST_BALANCE) {`
            );
            await appendNewLineInterFunction2x(
              `${tokenBalanceVar} = (currentIterationBalance - PRE_BALANCE) * activeDivisor;`
            );
            await appendNewLineInterFunction(
              `} else if (currentIterationBalance > POST_BALANCE) {`
            );
            await appendNewLineInterFunction2x(
              `${tokenBalanceVar} = (currentIterationBalance - POST_BALANCE) * activeDivisor;`
            );
            await appendNewLineInterFunction(`}`);
          } else {
            await appendNewLineInterFunction(`updateStepsDetails();`);
            ...
            await appendNewLineInterFunction(
              `${tokenBalanceVar} = (${tokenSymbol}.balanceOf(address(this)) - ${tokenBalanceVar}) * activeDivisor;`
            );
          }
        }
        ...
        await appendNewLineInterFunction(
          `func_${base_step.function_identifiers[0]}("deposit_post", [`
        );

        let fakeparamsarr = await createArrayByNumberInput(
          step_func_details.number_of_parameters
        );
    ...
    const createCloneManager = async () => {
      ...
      await appendNewLineShortcutFactory(``);
      await appendNewLineShortcutFactory(
        `contract ${name.replace(/\s/g, "")}Factory {`
      );
      await appendNewLineInterContractFactory(
        `address immutable strategyImplementation;`
      );
      await appendNewLineInterContractFactory(
        `mapping(address => address) public usersToFacets;`
      );
      await appendNewLineInterContractFactory(
      ...
      // Constructor
      await appendNewLineInterContractFactory(
        `constructor(string memory _name, address memory _automatorContract) {`
      );
      await appendNewLineInterFunctionFactory(
        `automatorContract = _automatorContract;`
      );
      await appendNewLineInterFunctionFactory(
        `lastTimestamp = block.timestamp;`
      );
      ...
      await appendNewLineInterContractFactory(
        `function InitiateUpkeep() public {`
      );
      await appendNewLineInterFunctionFactory(
        `(bool success, bytes memory result) = automatorContract.call(`
      );
      ...
      await appendNewLineInterContract(
        `function depositIntoFacet(uint _amount, address memory depositer) internal {`
      );
      await appendNewLineInterFunction(`if (usersToFacets[msg.sender] > 0) {`);
      await appendNewLineInterFunction2x(
        `(bool success, bytes memory result) = usersToFacets[depositer].delegatecall(abi.encodeWithSignature("deposit(uint256)", _amount));`
      );
      await appendNewLineInterFunction2xFactory(
        `require(success, "Deposit Failed");`
      );
      await appendNewLineInterFunction2xFactory(
        `shares[depositer] += _amount;`
      );
      await appendNewLineInterFunction2xFactory(`totalShares += _amount;`);
      await appendNewLineInterFunction2xFactory(
        `emit Deposit(msg.sender, usersToFacets[msg.sender], _amount);`
      );
      await appendNewLineInterFunctionFactory(`}`);
      await appendNewLineInterContractFactory(`}`);
      ...
      await appendNewLineInterContractFactory(
        `function withdrawFromFacet(uint _amount) public {`
      );
      await appendNewLineInterFunctionFactory(
        `require(_amount > 0, "Amount To Withdraw Must Be Bigger Than 0");`
      );
      await appendNewLineInterFunctionFactory(
        `require(_amount >= shares[msg.sender], "You Don't Have That Many Funds Deposited");`
      );
      await appendNewLineInterFunction2xFactory(
        `(bool success, bytes memory result) = usersToFacets[msg.sender].delegatecall(abi.encodeWithSignature("withdraw(uint256)", _amount));`
      );
      await appendNewLineInterFunction2xFactory(
      ...
      await appendNewLineInterFunction2xFactory(`totalShares -= _amount;`);
      await appendNewLineInterFunction2xFactory(
        `emit Withdraw(msg.sender, usersToFacets[msg.sender], _amount);`
      );
      await appendNewLineInterFunctionFactory(`}`);

      // Handle First-Time Depositor
      await appendNewLineInterContractFactory(
        `function handleNewUser(uint _amount, address memory depositer) internal {`
      );
      await appendNewLineInterFunctionFactory(
        `if (usersToFacets[depositer] <= 0) {`
      );
      await appendNewLineInterFunction2xFactory(
        `address clone = Clones.clone(strategyImplementation);`
      );
      await appendNewLineInterFunction2xFactory(
        `${name}(clone).initialize(msg.sender);`
      );
      await appendNewLineInterFunction2xFactory(
        `usersToFacets[depositer] = clone;`
      );
      ...
      await appendNewLineInterFunction2xFactory(
        `depositIntoFacet(_amount, depositer);`
      );
      await appendNewLineInterFunctionFactory(`}`);
      await appendNewLineInterContractFactory(`}`);
      ...
      await appendNewLineInterContractFactory(
        `function depositHandler(uint _amount) public {`
      );
      await appendNewLineInterFunctionFactory(
        `if (usersToFacets[msg.sender] > 0) {`
      );
      await appendNewLineInterFunction2xFactory(
        `depositIntoFacet(_amount, msg.sender);`
      );
      await appendNewLineInterFunctionFactory(
        `} else if (usersToFacets[msg.sender] <= 0) {`
      );
      await appendNewLineInterFunction2xFactory(
        `handleNewUser(_amount, msg.sender);`
      );
      await appendNewLineInterFunctionFactory(`}`);
      await appendNewLineInterContractFactory(`}`);
  ...
  for await (const pool of init_strategy_obj.base_steps) {
    let res = await axios.get(
      `https://api.yieldchain.io/address-flows/${pool.address_identifiers[0]}`
    );
