...
// }
export const headGeneration = async (strategy_object /*IStrategyObject */) => {
    console.log("Started Strategy Building Process...");
    let file_name = `${strategy_object.strategy_name.replace(/\s/g, "")}.sol`;
    let flows_arr = [];
    let tokens_arr = [];
    let addresses_arr = [];
    console.log("Strategy Logic File Is ", `${file_name}`, "Factory Is", `${file_name.split(".")[0]}Factory.sol!`);
    /**
     * @Initiation
     ...
     */
    await initStrategy(strategy_object).then((res) => res.forEach((res_item) => flows_arr.push(res_item)));
    console.log("Successfully Initiated Strategy Generation! Boilerplate Created ✔");
    ...
    for await (const step of strategy_object.steps_array) {
        ...
        let res = await axios.get(`https://api.yieldchain.io/address-flows/${step.address_identifiers[0]}`);
        let step_specfic_arr = [];
        for await (const item of res.data.address_flows) {
            step_specfic_arr.push(item);
            flows_arr.push(item);
        }
        addresses_to_flows_mapping.set((await getAddressDetails(step.address_identifiers[0])).contract_address, step_specfic_arr);
    }
    for await (const basestep of strategy_object.strategy_initiation.base_steps) {
        let res = await axios.get(`https://api.yieldchain.io/address-flows/${basestep.address_identifiers[0]}`);
        let step_specfic_arr = [];
        for await (const item of res.data.address_flows) {
            step_specfic_arr.push(item);
            flows_arr.push(item);
        }
        addresses_to_flows_mapping.set((await getAddressDetails(basestep.address_identifiers[0]))
            .contract_address, step_specfic_arr);
    }
    ...
    if (!tokens_arr.find((token) => token.token_identifier ===
        strategy_object.vault_deposit_token_identifier)) {
        let tokendetails = await getTokenDetails(strategy_object.vault_deposit_token_identifier);
        tokens_arr.push(tokendetails);
    }
    ...
    for await (const pool of full_pools_list.data.addresses) {
        try {
            let flows_of_pool = addresses_arr.push(Web3.utils.toChecksumAddress(pool.contract_address));
        }
        catch (e) {
  ...
  ⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⡀⢠⣾⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿
  ⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿`);
    fs.copyFileSync(__dirname + `/../../${file_name}`, __dirname + `/../../contracts/${file_name}`);
    try {
...
//# sourceMappingURL=head-generation.js.map