...
export const getFunctionDetails = async (_function_identifier) => {
    ...
    let full_functions_list = (await axios.get("https://api.yieldchain.io/functions")).data
        .functions; // Gets Full Functions List From Database
...
export const getParameterDetails = async (_parameter_identifier) => {
    let full_parameters_list = (await axios.get("https://api.yieldchain.io/parameters")).data
        .parameters; // Get full parameters list from API
...
export const getAddressDetails = async (_address_identifier) => {
    let full_addresses_list = (await axios.get("https://api.yieldchain.io/addresses")).data
        .addresses; // Gets the full list of addresses from the api
...
export const getFunctionAddress = async (_function_identifier) => {
    ...
    let currentAddress = allAddresses.find((addressObj) => addressObj.functions && addressObj.functions.includes(_function_identifier));
