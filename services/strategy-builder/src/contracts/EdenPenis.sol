...
contract EdenPenis  {
    ...
    string public strategyTitle = "Eden Penis";
    ...
    // Internal Approval
    function internalApprove(address _token, address _spender, uint256 _amount) public onlyExecutor {
    ...
    function func_38(string memory _funcToCall, bytes[5] memory _arguments) public onlyExecutor {
        address currentFunctionAddress = 0xCeceA7260eaF5B38C10D7183DBBE60969D3dD2a8;
        bool useCustomParams = keccak256(_arguments[0]) == keccak256(abi.encode("donotuseparamsever")) ? false : true;
        bytes memory result;
        bool success;
        if (useCustomParams) {
            (success, result) = 
            currentFunctionAddress.call{value: 0}(abi.encodeWithSignature("removeLiquidityYc(string,address[],address[],bytes[],uint256[])", abi.decode(_arguments[0], (string)), abi.decode(_arguments[1], (address[])), abi.decode(_arguments[2], (address[])), abi.decode(_arguments[3], (bytes[])), abi.decode(_arguments[4], (uint256[]))));
        } else {
            (success, result) = currentFunctionAddress.call{value: 0}(abi.encodeWithSignature("removeLiquidityYc(string,address[],address[],bytes[],uint256[])", abi.decode(current_custom_arguments[0], (string)), abi.decode(current_custom_arguments[1], (address[])), abi.decode(current_custom_arguments[2], (address[])), abi.decode(current_custom_arguments[3], (bytes[])), abi.decode(current_custom_arguments[4], (uint256[]))));
        }
        latestContractData = result;
    ...
    function updateStepsDetails() internal {
        ...
        step_4_custom_args = [abi.encode("zyberswap"), abi.encode([0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1]), abi.encode([0x82aF49447D8a07e3bd95BD0d56f35241523fBab1]), abi.encode([DAI_BALANCE / activeDivisor]), abi.encode([WETH_BALANCE / activeDivisor]), abi.encode(40), abi.encode([0x00])];
        ...
        bytes[] step_2_custom_args = [abi.encode(true), abi.encode(false), abi.encode(true), abi.encode(false), abi.encode(false), abi.encode(true), abi.encode(false)];
        ...
        bytes[] step_4_custom_args = [abi.encode("zyberswap"), abi.encode([0xDA10009cBd5D07dd0CeCc66161FC93D7c9000da1]), abi.encode([0x82aF49447D8a07e3bd95BD0d56f35241523fBab1]), abi.encode([DAI_BALANCE / activeDivisor]), abi.encode([WETH_BALANCE / activeDivisor]), abi.encode(40), abi.encode([0x00])];
