...
const generateFunctions = async (_fileName, _strategy_obj) => {
    ...
    let counterFunctionsArr = await Promise.all(steps_array.map(async (step) => {
        ...
        try {
            addressIdentifier = (await (0, utils_js_1.getFunctionAddress)(counterFunctionIdentifier.function_identifier)).address_identifier;
        }
        catch (e) {
    ...
    const fetchDetails = async (_arr) => {
        for await (const step of _arr) {
            ...
            for await (const funcid of step.function_identifiers) {
                let funct = await (0, utils_js_1.getFunctionDetails)(funcid);
                ...
                let funcaddress = await (await (0, utils_js_1.getAddressDetails)(step.address_identifiers[funcindex])).contract_address;
                let paramsArr = [];
                functions_arr.push(funct);
                for await (const paramid of funct.arguments) {
                    let param = await (0, utils_js_1.getParameterDetails)(paramid);
    ...
    for await (const func of functions_arr) {
        ...
        let func_address = web3_1.default.utils.toChecksumAddress(functions_to_addresses_mapping.get(func.function_identifier));
